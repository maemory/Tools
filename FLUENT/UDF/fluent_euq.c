 ...
 * Developed by: G. Iaccarino and M. Emory
 *
 * This fileis divided into sections. The first contains all general code needed
 * for the EUQ perturbations. Then subsequent sections contain the specific methods
 * required for a particular turbulence model
 */
...
enum {
	C1, C2, C3,                   /* desired barycentric convex coefficients */
	C1c,C2c,C3c,                  /* actual barycentric convex coefficients */
	D11, D12, D13, D22, D23, D33, /* modified reynolds stress terms */
	R11, R12, R13, R22, R23, R33, /* Reynolds stress terms */
	SENSOR_00,
	N_REQUIRED_UDM
};

/* -------------------------------------------------------------------------------
 * Abid Low-Reynolds number K-epsilon model
 * -------------------------------------------------------------------------------*/
...
double new_rsm[3][3];     /* perturbed reynolds stress */
...
/* User-defined scalars: k, epsilon */
enum {
	K, D, N_REQUIRED_UDS
};

/*----------------------------------------------------------------*/
/* General EUQ functions  - start             					  */
/*----------------------------------------------------------------*/
...
void compute_barycentric_map(cell_t c, Thread *t, double my_tke, double my_rsm[3][3]) {
	...
	bary_coordinates[0] = corner[0][0] * c1c + corner[1][0] * c2c
			+ corner[2][0] * c3c;
	bary_coordinates[1] = corner[0][1] * c1c + corner[1][1] * c2c
			+ corner[2][1] * c3c;
	...
	for (i = 0; i < 3; i++) {
		for (j = 0; j < 3; j++) {
			new_rsm[i][j] = 2.0 * new_tke
					* (a_ij[i][j] + 1.0 / 3.0 * delta_ij[i][j]);
		}
	}
...
DEFINE_PROFILE( wall_d_bc, t, position) {
	...
	begin_f_loop(f, t)
	{
		...
		drootkdy = rootk / dy;
		F_PROFILE(f, t, position) = 2. * C_MU_L(c0, t0) / C_R(c0, t0) * drootkdy
				* drootkdy;
	}
	end_f_loop(f, t)
}
...
/*----------------------------------------------------------------*/
/* Abid k-epsilon turbulence model functions                      */
/*----------------------------------------------------------------*/
...
double f_mu(cell_t c, Thread *t) {
	...
	my_fmu = tanh(0.008 * Re_y(c, t))
							* (1.0 + 4.0 / pow(MAX(Re_t(c, t), SMALL_NUMBER), 0.75));
...
void compute_abid_rsm_ij(cell_t c, Thread *t) {
	...
	for (m = 0; m < 3; m++) {
		for (n = 0; n < 3; n++) {
			double mu_t = ev(c, t);
			abid_rsm[m][n] = 2.0 / 3.0 * abid_tke * delta_ij[m][n]
			                                                    - 2.0 * mu_t / C_R(c, t) * s_ij[m][n];
		}
	}
}
...
DEFINE_ADJUST( abid_adjust, domain) {
	...
	int m, n;
	double eigv[3][3], eigs[3], a_ij[3][3];
	double corner[3][3];
	...
	begin_c_loop(c, t)
	{
		/* compute abid reynolds stress */
		compute_abid_rsm_ij(c, t);

		/* pertrub reynolds stress */
		...
		/* compute enforced reynolds stress anisotropy tensor */
		for (m = 0; m < 3; m++) {
			for (n = 0; n < 3; n++) {
				a_ij[m][n] = 0.5 * (URLX * new_rsm[m][n] + (1-URLX) * abid_rsm[m][n]) / abid_tke - delta_ij[m][n] / 3.;
			}
		}
		...
		/* compute convex combination coefficients */
		C_UDMI(c, t, C1c) = eigs[2] - eigs[1];
		C_UDMI(c, t, C2c) = 2.0 * (eigs[1] - eigs[0]);
		C_UDMI(c, t, C3c) = 3.0 * eigs[0] + 1.0;
...
DEFINE_ADJUST( rsm_adjust, domain) {
	...
	begin_c_loop(c, t)
	{
		...
		tmp_rsm[0][0] = C_RUU(c, t);
		tmp_rsm[1][1] = C_RVV(c, t);
		tmp_rsm[2][2] = C_RWW(c, t);
		tmp_rsm[0][1] = C_RUV(c, t);
		tmp_rsm[0][2] = C_RUW(c, t);
		tmp_rsm[1][2] = C_RVW(c, t);
		tmp_rsm[1][0] = tmp_rsm[0][1];
		tmp_rsm[2][0] = tmp_rsm[0][2];
		tmp_rsm[2][1] = tmp_rsm[1][2];
		tmp_tke = 0.5 * (C_RUU(c, t) + C_RVV(c, t) + C_RWW(c, t));

		/* compute enforced reynolds stress anisotropy tensor */
		for (m = 0; m < 3; m++) {
			for (n = 0; n < 3; n++) {
				a_ij[m][n] = 0.5 * tmp_rsm[m][n] / tmp_tke - delta_ij[m][n] / 3.;
			}
		}
