/*=================================================================
 *
 * GEN_HWYGLYPH_magnitude.cpp	Sample .MEX file corresponding to gen_HWYglyph_magnitude.M
 *	        Solves for Reynolds stress tensor glyph given stress tensor and points at which to evaluate magnitude
 *
 * The calling syntax is:
 *
 *		[X,Y,Z,C] = gen_HWYglyph_magnitude(Rij,points)
 *
...
static void make_glyph(const double *rij, const int dimi,const int dimj, double *xo,double *yo,double *zo,double *co,double *X,double *Y,double *Z) {
  ...
  double x,y,z,mag,T1,T2,T3,norm,base;
  ...
  for (int i=0; i<dimi; i++) {
    for (int j=0; j<dimj; j++) {
      ...
      norm = x*x*rij(0,0) + y*y*rij(1,1) + z*z*rij(2,2) + 2*x*y*rij(0,1) + 2*x*z*rij(0,2) + 2*y*z*rij(1,2);

      T1 = rij(0,0)*x + rij(0,1)*y + rij(0,2)*z;
      T2 = rij(1,0)*x + rij(1,1)*y + rij(1,2)*z;
      T3 = rij(2,0)*x + rij(2,1)*y + rij(2,2)*z;
      
      base = T1*T1 + T2*T2 + T3*T3 - norm*norm;
