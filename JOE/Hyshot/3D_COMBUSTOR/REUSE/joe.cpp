...
//#include "combModels/CombModel_MixBurn.h"
...
/* ###########################################################################################
   ...
   - Menter Gamma-Re transition-like implementation lives here, certain parameters
   such as wall distance to use for specific regions are hard coded

   #########################################################################################*/
class JoeSST : public JoeWithModels, public RansTurbKOmSST
{
public:
...
//  double x_transition, y_transition;
  ...
  double x_body, x_cowl;			///< locations where transition occurs along geometry

public:
  JoeSST(char *name) : JoeWithModels(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0)
      cout << "JoeSST()" << endl;
  ...
  virtual void initialHook()
  {
    ...
    // Set BL transition scaling flag only once
...
//    if (mpi_rank == 0)
//      cout << "JoeSST::initialHook: setToLaminar for (x,y)<(" << x_transition << "," << y_transition << ")" << endl;
  }
  ...
  virtual void sourceHookRansTurbCoupled(double **rhs, double ***A, int nScal, int flagImplicit)
  {
    ...
    {
      /*-----------------------------------
       * k-equation specific modifications
       */
      double zeta = min(1.0/omega[icv], a1/(limiterFunc[icv]*blendFuncF2[icv]));
      ...
      /* enforce transition according to Menter gamma-Re_theta criterion:
		 ...
		 - damping effect on dissipation term in k-equation [0.1-1]
	   */
      rhs[icv][5+kine_Index]  += (trans_scale[icv]*Pk - (0.1+0.9*trans_scale[icv])*betaStar*rho[icv]*omega[icv]*kine[icv])*cv_volume[icv];

      if (flagImplicit)
      {
        int noc00 = nbocv_i[icv];
        A[noc00][5+kine_Index][5+kine_Index]   -= - (0.1+0.9*trans_scale[icv])*betaStar*omega[icv]*cv_volume[icv];
      }

      /*-----------------------------------
	   * omega-equation specific modifications
	   */
      double F1 = blendFuncF1[icv];
      SST_F1[icv] = F1;
...
/* ###########################################################################################
   Class JoeSST_mixing:
   - mixing without combustion

   #########################################################################################*/
class JoeSST_mixing : public JoeSST, public RansCombMixing
{
public:

  JoeSST_mixing(char *name) : JoeSST(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0) cout << "JoeSST_mixing()" << endl;
  }

  virtual ~JoeSST_mixing()  {}
...
/* ###########################################################################################
   Class JoeSST_MixBurn:
   - Mix & Burn combustion model shouldbe implemented here (current stuff is broken)
   - should be similar to mixing model above, with added source in energy equation

   #########################################################################################*/
class JoeSST_MixBurn : public JoeSST , public RansCombMixing // public RansCombMixBurn
{
public:
...
#define NVAL 7
  ...
  JoeSST_MixBurn(char *name) : JoeSST(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0) cout << "JoeSST_MixBurn()" << endl;
  ...
  virtual ~JoeSST_MixBurn()  {}

  void initialHook()
  {
    JoeSST::initialHook();

    if (mpi_rank == 0)  cout << "JoeKOM_MixBurn::initialHook()"<< endl;

    // count to make sure only one HOOK boundary defined, important in BoundarkHook
    int nhooks=0;
    Param *param;
    for (list<FaZone>::iterator zone = faZoneList.begin(); zone != faZoneList.end(); zone++)
      if (zone->getKind() == FA_ZONE_BOUNDARY)
...
/* ###########################################################################################
   Class JoeSST_fpva_coeff:
   - Add FPVA combustion model

   #########################################################################################*/
//class JoeSST_fpva_coeff : public JoeSST, public RansCombFPVA_Coeff<ChemtableAdaptiveLinear>
class JoeSST_fpva_coeff : public JoeSST, public RansCombFPVA_Coeff<ChemtableCartesianLinear>
{
public:
  JoeSST_fpva_coeff(char *name) : JoeSST(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0) {
      cout << "JoeSST_fpva_coeff()" << endl;
  ...
  virtual ~JoeSST_fpva_coeff()  {}

  void initialHook()
  {
    JoeSST::initialHook();

    if (mpi_rank == 0)  cout << "JoeSST_fpva_coeff::initialHook()"<< endl;

    /*
     *  Intitialization of FPVA variables if not set already
     */
    if (!checkScalarFlag("ZMean")) {   
      if (mpi_rank==0)  cout << "Set ZMean=0 everywhere" << endl ;
      for (int icv = 0; icv < ncv; icv++)
  ...
  {
    if (mpi_rank==0)  cout << "JoeSST_fpva_coeff::integrateSources(" << dxbin << "," << dybin << "," << dzbin << ")" << endl ;
    //  Find min/max x,y,z
...
//#define _FIX_OUTLET_IN_VINCEGRID_    //  Vince's grid has unified outlet (bleed and actual outlet) -- fix by adding extra bnd...
    ...
    if (mpi_rank==0)  cout << "JoeSST_fpva_coeff::computeBoundaryMassfluxes()" << endl ;
  ...
  {
    if (mpi_rank==0)  cout << "JoeSST_fpva_coeff::extractProfiles()" << endl ;
    double xmin[3] = { -1.0e10 , -1.0e10 , -1.0e10 } ;
    double xmax[3] = {  1.0e10 ,  1.0e10 ,  1.0e10 } ;
...
/* ###########################################################################################
   Class hyshot_fpva_coeff:
   - hyshot specific class

   #########################################################################################*/
class hyshot_fpva_coeff : public JoeSST_fpva_coeff
{
public:
...
#define NVAL 7
  ...
  hyshot_fpva_coeff(char *name) : JoeSST_fpva_coeff(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0) {
  ...
  {
	/*
	 * Interpolate profile to set inflow noundary condition
	 */
    int pos=1;
  ...
  virtual void boundaryHookScalarRansTurb(double *phi_fa, FaZone *zone, const string &name)
  {
	  ...
	  //        }
	  //      }
	  if ((name == "kine") && (zone->getNameString() == "inlet"))
		  for (int ifa=zone->ifa_f; ifa<=zone->ifa_l; ifa++)
			  phi_fa[ifa] = getValues(x_fa[ifa][1], 3);

	  if ((name == "omega") && (zone->getNameString() == "inlet"))
		  for (int ifa=zone->ifa_f; ifa<=zone->ifa_l; ifa++)
			  phi_fa[ifa] = getValues(x_fa[ifa][1], 4);
  ...
  virtual void initialHook()
  {
    JoeSST_fpva_coeff::initialHook();
    //  Verify that only a single boundary has been defined as HOOK
    //  Otherwise will get trouble in boundaryHook below...
    if (mpi_rank==0)  cout << "hyshot_fpva_coeff::initialHook: counting number of HOOK boundaries..." ;
    ...
    //  Set initial field from inflow profile
   if (!checkScalarFlag("RHO")) {
   	if (mpi_rank==0)  cout << "Setting initial N-S fields from inflow profile..." ;
...
// ###########################################################################################
class hyshot_mixing : public JoeSST_mixing
{
public:
...
#define NVAL 7
  ...
  hyshot_mixing(char *name) : JoeSST_mixing(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0) {
...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    switch (run)
    {
      case 1:  joe = new JoeSST(inputFileName);      break;
      case 2:  joe = new JoeSST_fpva_coeff(inputFileName);  break;
      case 3:  joe = new hyshot_fpva_coeff(inputFileName);  break;
//      case 4:  joe = new hyshot_mixing(inputFileName);      break;
//      case 5:  joe = new JoeSST_MixBurn(inputFileName);     break;
